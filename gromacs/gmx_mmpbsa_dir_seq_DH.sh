...
#    请检查pdb文件确保构型PBC处理正确
# 1. pre-processe trajectory, whole, cluster, center, fit, then generate pdb file
## -pbc whole 只使破碎的分子完整, mol将分子的质心放在框中，并需要提供一个运行输入文件-s
## -fit Fit molecule to ref structure in the structure file: none, rot+trans, rotxy+transxy, translation, transxy, progressive
...
awk -v pid=_$pid          -v qrv=$qrv           -v dt="$dt"     \
	...
	/REMARK/ {next}    #/REMARK/是正则匹配，匹配有REMARK的行 
	/TITLE/ {
		if(Nfrm) {
	...
	END{
		...
		for( i in ndxPro) dE[resPro[i]]=0
		for( i in ndxLig) dE[resLig[i]]=0
		...
		for(fr=1; fr<=Nfrm; fr++) {
			...
			if(withLig) {
			    ...
			    for( ii in ndxPro) {
					...
					for( jj in ndxLig) {
						...
						if(r==0) {
							printf "------------- ii = %d jj = %d--------\n",ii,jj
							printf "------------- xi = %9.3f x[jj] = %9.3f--------\n",xi,x[jj]
							printf "------------- xi = %9.3f  yi = %9.3f  zi = %9.3f--------\n",xi,yi,zi
						}
						#printf "----non zero-- ii = %d jj = %d--------\n",ii,jj
						if(r<Rcut) {
							t=1/(.1*r)^6
							Ecou   = qi*Qatm[jj]/r;
			...
			for(i in ndxPro) { PBres[resPro[i]]=0; SAres[resPro[i]]=0 }
			for(i in ndxLig) { PBres[resLig[i]]=0; SAres[resLig[i]]=0 }
            
			for(i in ndxPro) {
				PBres[resPro[i]] += Esol[1, i]-Esol[2, i]
				SAres[resPro[i]] += Esas[1, i]-Esas[2, i]
			}
	
			for(i in ndxLig){
				PBres[resLig[i]] += Esol[1, i]-Esol[3, i]
			...
			for(i in ndxPro) {
				txt=sprintf("%-6s%5d %-4s %3s A%4d    %8.3f%8.3f%8.3f", \
					"ATOM", i, type[i], res[i], resID[i], x[i], y[i], z[i])
				if(withLig) {
