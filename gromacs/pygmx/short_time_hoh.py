...
def apply_windows(xtc, gro, R_idx, L_idx, win_params, num_hyHOH, thr=0.4):
    ...
    for (start, end) in windows(begin, final, win_len, win_stride):
        ...
        gmx.make_ndx(f=gro, o=ndx, input='q')
        ...
        gmx.rmsf(s=gro, f=xtc, ox=short_ave_pdb, b=start, e=end, n=ndx, input='System')
        ...
        RHOHs, LHOHs = assign_water(protein_atoms, hy_HOHs)
        ...
        hyHOH_list.sort()
        gmx.make_ndx(f=gro, n=ndx, o=ndx, input=('ri ' + ' '.join(str(hoh) for hoh in hyHOH_list),
                                                 'name 19 hyHOH',
                                                 '1 | 19',
                                                 'name 20 com', 'q'))  # 19
        # print('"Protein" | ri ' + ' '.join(str(hoh) for hoh in hyHOH_list))
        gmx.trjconv(f=xtc, o=short_xtc, b=start, e=end, n=ndx, input='20')
        os.system('rm ' + short_ave_pdb)
        gmx.rmsf(s=gro, f=xtc, ox=short_ave_pdb, b=start, e=end, n=ndx, input='20')

        grp_RHOHs = 'r_' + '_'.join(str(hoh) for hoh in RHOHs)
        grp_LHOHs = 'r_' + '_'.join(str(hoh) for hoh in LHOHs)
        select_RH_cmd = 'ri ' + ' '.join(str(hoh) for hoh in RHOHs)
        select_LH_cmd = 'ri ' + ' '.join(str(hoh) for hoh in LHOHs)
        ...
        gmx.make_ndx(f=short_ave_pdb, o=short_ndx, input=(select_R_cmd, select_L_cmd,  # 19 20
                                                          select_RH_cmd, select_LH_cmd, select_com_cmd,
                                                          'q'))  # 21 22 23
        ...
        gmx.make_ndx(f=short_ave_pdb, n=short_ndx, o=short_ndx, input=(select_RPW_cmd, select_LPW_cmd, 'q'))  # 24 25

        # grp_W = 'r_' + '_'.join(str(hoh) for hoh in hy_HOHs_idx)
        # select_command = 'ri ' + ' '.join(str(hoh) for hoh in hy_HOHs_idx)
        ...
        command = '/media/xin/WinData/ACS/gmx/gmx_mmpbsa_ed.bsh' \
                  ...
                  + ' -pro ' + grp_R_pw \
                  + ' -lig ' + grp_L_pw \
                  + ' -b ' + str(int(start + 20)) + ' -e ' + str(int(end - 20)) + ' -i 10' \
                  + ' -cou dh -ts ie'
        os.system(command)
