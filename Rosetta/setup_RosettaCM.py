...
if __name__ == "__main__":
    ...
    parser.add_argument('--alignment_format', action="store", default="grishin",
                        help='input alignment file format (grishin, modeller, hhsearch, clustalw, or fasta)')
    parser.add_argument('--templates', nargs='*', default=[], help="input target templates")
    parser.add_argument('--rosetta_bin', action="store", default="~/env/rosetta_src_2020.37.61417_bundle/main/source/bin", help="rosetta path")
    ...
    parser.add_argument('--compiling_mode', action="store", default="release",
                        help="compiling mode name in rosetta executable")
    ...
    parser.add_argument('-j', action="store", type=int, default=1,
                        help="number of processors (requires GNU parallel in path!)")
    ...
    if len(args.setup_script.strip()) > 0:
        ...
        if os.path.exists(args.setup_script):
            ...
            for flag in flags:
                command = "%s @flags_common @%s -database %s -nstruct 10" % (
                    exe_name, os.path.basename(flag), rosetta_db)
                if not args.run:
    ...
    if (not os.path.exists("hybrid/hybridize0_C1.xml")):
        ...
        # get partial thread
        exe_name = "%s/partial_thread.%s.%s%s%s" % (
            args.rosetta_bin, args.build, args.platform, args.compiler, args.compiling_mode)
        command = "%s -database %s -mute all -in:file:fasta %s -in:file:alignment %s -in:file:template_pdb %s -ignore_unrecognized_res" % (
            exe_name, rosetta_db, fasta_fn, converted_aln, template_filenames)
        ...
        if args.j == 1:
            ...
            command = "parallel -j %d %s @%s -database %s -nstruct %d -out::suffix _{} ::: {1..%d}" % (
                args.j, exe_name, flag_fn, rosetta_db, math.ceil(20 / args.j), args.j)
